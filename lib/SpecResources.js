    ...
    this
      ...
      .pipe(concat(function (data) {
        resolve({
          ...
          uniqueFilename: source.path.replace(/\//g, '__').toLowerCase(),
          data: data.toString()
        });
      }));
  }.bind(this));
...
/**
 * Returns contents of source code file referenced in spec annotation
 * @method getCodeFile
 * @param {number} idx
 * @returns {Promise}
 */
SpecResources.prototype.getCodeFile = function (filename) {
  return new Promise(function (resolve, reject) {
    this.specObj.load().then(function () {
      this.specObj.ctx().then(function (ctx) {
        var foundCodeFile = false;

        ctx.code.some(function (code) {
          if (code.uniqueFilename.match(new RegExp(filename, 'i'))) {
            foundCodeFile = true;
            resolve(code.data);
            return true;
          }
        ...
        if (!foundCodeFile) {
          reject(new Error('Unable to locate code file'));
        }
      });
    }.bind(this));
  }.bind(this));
...
/**
 * Returns list of code files referenced by the spec
 * @method getCodeFile
 * @param {number} idx
 * @returns {Promise}
 */
SpecResources.prototype.getCodeFileList = function (filename) {
  return new Promise(function (resolve, reject) {
    this.specObj.load().then(function () {
      this.specObj.ctx().then(function (ctx) {
        resolve(ctx.code);
