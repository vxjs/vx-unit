...
function SpecStore(plugin) {
  ...
  // Route serves the sandbox page for a single spec file
  router.addRoute(/^\/spec\/browser\/sandbox\/(.*)\/?$/i, this.sandboxPage, this);

  // Route serves the test harness page for a single spec file
  // (spec is executed in browser)
  router.addRoute(/^\/spec\/browser\/(.*)\/?$/i, this.browserHarnessPage, this);

  // Route serves information about the code annotations for a single spec file
  router.addRoute(/^\/spec\/(.*)\/code\/?$/i, this.serveCodeFileList, this);

  // Route serves the transformed source code for a single spec file
  router.addRoute(/^\/spec\/(.*)\/code\/(.*)\/?$/i, this.serveCodeFile, this);

  // Route serves the server test harness page for a single spec file
  // (spec is executed on server)
  router.addRoute(/^\/spec\/server\/(.*)\/?$/i, this.serverHarnessPage, this);

  // Route serves a list of all loaded specs
